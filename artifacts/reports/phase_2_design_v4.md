# Phase 2 Design Specification: Market Feature Engine (Revision 4)

**Project**: XAU Quantitative Strategy Discovery and Decision Platform  
**Phase**: 2 (Market Feature Engine)  
**Revision**: 4 — resolving all remaining specification ambiguities, semantic boundaries, and acceptance criteria refinements  
...
## Revision 4 Summary of Refinements

This document supersedes Revision 3. The following refinements have been applied in strict accordance with the final review:

| # | Item | Revision 4 Resolution | Document Section |
|---|---|---|---|
| **R1** | **$I(t)$ information set conflicted with 1m MTF bars** | Defined $I(t)$ strictly by **observation close/completion timestamp** ($\le t + 300\text{s}$) rather than candle open timestamp, explicitly validating the five constituent 1m bars $\{t, t+60\text{s}, \dots, t+240\text{s}\}$. | Section 5.1 |
| **R2** | **Zero-volume epsilon policy underspecified** | Replaced silent `+ eps` in liquidity metrics with an explicit semantic zero-volume policy table: zero-volume denominators deterministically evaluate to `NaN` (or `0.0` for return skew boundary). | Section 3.4, Section 3.8.3 |
| **R3** | **Garman–Klass non-negative variance guard** | Specified explicit non-negative variance clamping `variance = max(raw_var, 0.0)` prior to `sqrt()` to guard against floating-point roundoff below zero while distinguishing this from invalid OHLC data. | Section 3.3.3 |
| **R4** | **`nan_feature_count` triggered by nullable event columns** | Defined `nan_feature_count` strictly over **non-nullable numeric feature outputs**. Explicitly excluded nullable pivot event columns (`timestamp` and `age_bars`), whose `NULL` values when `is_pivot == False` are semantically valid. | Section 6.2, Section 8.2 |
| **R5** | **Hurst naming and qualification consistency** | Consistently designated as **single-window $R/S$ proxy** (`mr_hurst_proxy_n`), explicitly documenting that it is a research proxy and not a multi-scale estimator. | Section 3.6.3 |
| **R6** | **Determinism requirements decoupled** | Clarified the distinct testing targets: same supported runtime + writer configuration requires **byte-identical Parquet**, while cross-runtime/architecture requires **numerical equivalence within tolerance** ($10^{-12}$ general, $10^{-10}$ recursive accumulators). | Section 5.3, Section 11 |
| **R7** | **AC-6 upgraded from arbitrary 99.9% budget** | Replaced the 0.1% failure budget with the project's exact data-quality standard: **all post-warm-up rows must be valid except those explicitly affected by registered input gaps or incomplete source observations**, with full diagnostic accounting. | Section 6.2, Section 11 |
...
| Determinism contract | **Byte-identical on same runtime; numerical equivalence ($10^{-12}$ / $10^{-10}$) cross-runtime** |
...
#### 3.3.3 Garman-Klass Volatility Estimator (Non-Negative Variance Guard, Refinement R3)
```
GK[t] = 0.5 * (ln(H[t] / L[t]))^2 - (2.0 * ln(2.0) - 1.0) * (ln(C[t] / O[t]))^2
raw_variance = (1.0 / n) * sum_{k=0}^{n-1} GK[t-k]
```
> [!IMPORTANT]
> **Refinement R3: Non-Negative Variance Guard**:
> In theory $GK[t] \ge 0.0$, but floating-point arithmetic can produce an infinitesimal negative value (e.g. $-10^{-17}$) when $O \approx H \approx L \approx C$.
> The implementation must strictly execute:
> ```python
> if not (H[t] >= L[t] > 0.0 and O[t] > 0.0 and C[t] > 0.0):
>     vol_gk_realized = float("nan")  # Invalid OHLC input
> else:
>     variance = max(raw_variance, 0.0)  # Floating-point non-negative guard
>     vol_gk_realized = math.sqrt(variance)
> ```
...
### 3.4 Volume and Liquidity Family (Explicit Zero-Volume Policy, Refinement R2)

> [!IMPORTANT]
> **Refinement R2: Zero-Volume Denominator Semantics**:
> Silent epsilon addition is prohibited in liquidity metrics where zero volume constitutes a semantic boundary. When trading volume is zero, metrics evaluate deterministically according to the following specification:
...
#### 3.6.3 Single-Window Rescaled Range (R/S) Hurst Proxy (Refinement R5)

> [!IMPORTANT]
> **Refinement R5: Qualification as Single-Window R/S Proxy**:
...
#### 3.8.2 Derivation of MTF `required_history_bars`
...
6. `mtf_15m_liq_vol_ratio_16`: $K=16 \implies \text{req} = 3 \cdot 16 + 2 = \mathbf{50}$ 5m bars (outputs `NaN` if $\text{mean}(V_{\text{15m}}) == 0.0$).
...
#### 3.8.3 MTF 1m Microstructure Context (Refinements R1, R2)
...
| `vol_gk_realized_{12,24}` | volatility | DOUBLE | $n$ | $n-1$ | Single-bar GK with non-negative guard |
| `vol_zscore_12_96` | volatility | DOUBLE | 108 | 107 | $13 + 96 - 1$ |
| `liq_taker_buy_ratio` | liquidity | DOUBLE | 1 | 0 | Single bar TB/V (NaN if V=0) |
| `liq_avg_trade_size` | liquidity | DOUBLE | 1 | 0 | Single bar V/N (NaN if N=0 or V=0) |
| `liq_vwap_dev_{12,48}` | liquidity | DOUBLE | $n$ | $n-1$ | Rolling QV/V (NaN if sum(V)=0) |
| `liq_vol_ratio_{12,24,48}` | liquidity | DOUBLE | $n$ | $n-1$ | Rolling mean V (NaN if mean(V)=0) |
| `liq_dollar_volume` | liquidity | DOUBLE | 1 | 0 | Re-exposed quote volume |
| `liq_delta_proxy` | liquidity | DOUBLE | 1 | 0 | Single bar delta (NaN if V=0) |
| `liq_cum_delta_{12,48}` | liquidity | DOUBLE | $n$ | $n-1$ | Rolling delta (NaN if sum(V)=0) |
...
| `mtf_15m_liq_taker_buy_ratio` | mtf | DOUBLE | 5 | 4 | $3 \cdot 1 + 2$ (NaN if $V_{\text{15m}}=0$) |
| `mtf_15m_liq_vol_ratio_16` | mtf | DOUBLE | 50 | 49 | $3 \cdot 16 + 2$ (NaN if $\text{mean}(V)=0$) |
...
| `mtf_1h_liq_taker_buy_ratio` | mtf | DOUBLE | 23 | 22 | $12 \cdot 1 + 11$ (NaN if $V_{\text{1h}}=0$) |
...
### 5.1 No Look-Ahead Principle (Refinement R1)

> [!IMPORTANT]
> **Refinement R1: Completion-Timestamp Information Set**:
...
### 5.3 Determinism and Reproducibility (Refinement R6)

The engine enforces two distinct determinism targets:
1. **Same Supported Runtime**:
   - Environment: Python 3.12, identical DuckDB version, identical OS.
   - Requirement: **Byte-identical Parquet files** producing matching file-level and dataset-level SHA-256 digests across repeated executions.
2. **Cross-Architecture / Cross-Runtime Equivalence**:
   ...
   - Requirement: **Numerical equivalence within defined tolerances**:
     - General scalar features: $|a - b| \le 10^{-12}$
     - Recursive accumulators (EMA, ATR, Wilder RSI, Welford std): $|a - b| \le 10^{-10}$
     - Discrete / time features: $a == b$ exactly
   - **Row ordering and schema**: Byte-for-byte identical in all environments.

### 5.4 Missing Candle & Gap-Invalidation Policy
1. **Zero Backward Extension**: Lookback windows strictly cover $n \times 300\text{s}$ of wall-clock time. If any bar in the window is missing from the canonical dataset, the feature outputs `NaN`.
2. **Stateful Accumulator Reset**: If a gap occurs, recursive filters pause updates and output `NaN` until re-seeded with a continuous block of $n$ bars.
3. **Absence of Aligned HTF Candle**: If the completed HTF candle required by the alignment rule is missing, all dependent MTF features evaluate to `NaN`.

---

## 6. Feature Quality Controls & Row Validity Flags
...
### 6.2 Decoupled Validity Model & Precision Accounting (Refinements R4, R7)

To prevent conflating history warm-up with data corruption or misinterpreting valid event `NULL`s:

> [!IMPORTANT]
> **Refinement R4: Precise `nan_feature_count` Definition**:
> $$\text{nan\_feature\_count} = \text{count of non-nullable numeric feature columns whose value is NaN in this row}$$
> Nullable event columns (`ms_pivot_high_timestamp_{s}`, `ms_pivot_high_age_bars_{s}`, `ms_pivot_low_timestamp_{s}`, `ms_pivot_low_age_bars_{s}`) are **excluded** from `nan_feature_count`. Their `NULL` values when `is_pivot == False` are semantically valid.

> [!IMPORTANT]
> **Refinement R7: Row Validity & Diagnostic Accounting**:
> - **`is_warmup: BOOLEAN`**:
...
> - **Zero-Unexplained-Loss Standard**:
>   All post-warm-up rows must have `is_valid = True` **except rows explicitly affected by registered input gaps or incomplete source observations**.
>   The manifest coverage section must account for every invalid row:
>   ```json
>   "coverage": {
>     "total_5m_bars": 315648,
>     "max_warm_up_bars": 311,
>     "total_post_warmup_rows": 315337,
>     "valid_rows": "<exact_count>",
>     "invalid_rows": "<exact_count>",
>     "invalid_row_reasons": {
>       "registered_gap_invalidation": "<count>",
>       "corrupt_or_missing_candle": 0
>     },
>     "gap_intervals_responsible": ["<iso_interval_1>", "..."]
>   }
>   ```
...
- `nan_feature_count`: `SMALLINT` (Count of NaN values in non-nullable numeric feature columns)
...
- `test_garman_klass_non_negative_guard`: verify tiny negative raw variance rounds cleanly to 0.0 without crash or NaN.
- `test_zero_volume_liquidity_nans`: verify zero-volume candle produces `NaN` for `liq_taker_buy_ratio`, `liq_avg_trade_size`, `liq_delta_proxy`, etc.
- `test_zscore_zero_variance_is_zero`: constant close series yields Z-score = $0.0$.
- `test_hurst_proxy_synthetic_series`: evaluate frozen R/S proxy on persistent series ($H > 0.5$) and random walk ($H \approx 0.5$).
...
## 11. Acceptance Criteria (Refinement R7)
...
| AC-7 | Identical serialized Parquet output across repeated runs on same supported runtime; numerical equivalence within tolerance across platforms | Byte-level SHA-256 comparison on same runtime; cross-platform test harness |
